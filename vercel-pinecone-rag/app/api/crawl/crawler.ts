...
class Crawler {
  ...
  async crawl(startUrl: string): Promise<Page[]> {
    ...
    while (this.shouldContinueCrawling()) {
      ...
      // If the depth is too greate or we've already seen this URL, skip it
      ...
      // Extract new URLs form the page HTML and add them to the queue
  ...
  private addToQueue(url: string, depth = 0) {
    return this.queue.push({ url, depth });
  }
  ...
  private isTooDeep(deep: number) {
    return deep > this.maxDepth;
  }
